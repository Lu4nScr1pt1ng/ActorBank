...
signed bearer tokens at the API edge. Single-threaded access means `Register` and
...
### Authentication at the stateless edge
Auth runs entirely at the API edge, off the grain path. A `CredentialGrain` (one per user — single-threaded,
so `Register` and `Authenticate` can never race) holds a salted **PBKDF2-SHA256** hash; `POST /auth/token`
verifies it and issues a compact, signed bearer token (ML-DSA-65). Every `/accounts` request is then
authorized **without touching a grain**:

- **Signature verify, cached.** `PqcBearerHandler` checks the token's signature on each request, but a
  bounded in-memory cache of already-verified tokens lets hot paths skip the re-check (a signature
  verification is far costlier than a cache hit), and a per-thread (`ThreadLocal`) verifier keeps that
  check fully concurrent across requests.
...
- **Auth** — credentials issue signed bearer tokens; `/accounts` requires a valid
...
interest sweep, and signed-token auth with a verification cache. Reaching 100B/day is **more of the
...
│  ├─ Accounts/  AccountGrain, AccountState, AccountGrainLog,
│  │             AccountReadModelGrain, AccountReadModelState (balance read model),
│  │             InterestSweepGrain, InterestSweepState (sharded interest sweep)
...
│  ├─ Auth/            PqcTokenService, PqcTokenOptions, PqcBearerHandler, PqcBearerDefaults, AccountOwnershipFilter
│  ├─ Contracts/       AccountRequests, AccountResponses, AuthContracts
│  ├─ Infrastructure/  OrleansConfiguration, BankExceptionHandler, InterestOptions, OpenApiSecurity
...
traces let you follow a transfer across grains. The API is pinned to **`:8080`**, so the k6 tests run
against Aspire unchanged — `./tests/run.sh smoke.js` (§10).
...
> The host enables Server GC + Concurrent GC + Tiered PGO (`ActorBank.Api.csproj`) — the right profile
> for a long-running silo.
...
- **xUnit ACID suite** (`tests/ActorBank.Tests`) — runs against a real in-memory Orleans cluster with
  ...
  (incl. the page-boundary flush), interest, the interest-shard mapping, and the balance read model
  (22 cases). Run with `dotnet test`.
